...
public class TSP extends
    ...
    public void compute(Iterator<ArrayListWritable<Text>> msgIterator) {
        ...
        if(k==0){
        	...
        	//Superstep 0 : we send only from the source vertex
        	if(this.isSource()){
        		for (LongWritable targetVertexId : this) {
        			Message msg=new Message();
        			String trajetIni=this.getVertexId().toString();
        			//we create the message
        			msg.add(new Text(trajetIni));
					msg.add(new Text(this.getEdgeValue(targetVertexId).toString()));
					//we actually send the message
					this.sendMsg(targetVertexId,msg);
					System.out.println("sending the message "+msg.toString()+" to the vertex "+targetVertexId.toString());
					Valeur valeur = new Valeur();
					valeur.add(new DoubleWritable(0));
					this.setVertexValue(valeur);
        		}	
        	}
        	else{
        		System.out.println("Nothing to do during this superstep : this vertex is not the source vertex");
        	}
        System.out.println("END OF SUPERSTEP 0");
        }
